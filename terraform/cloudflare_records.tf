...
resource "cloudflare_dns_record" "record" {
  ...
  name            = "${each.value.hostname}.${data.cloudflare_zone.bowmans_site.name}"
  type            = "A"
...
resource "cloudflare_dns_record" "alias" {
  ...
  type            = "CNAME"
  comment         = try(each.value.description, each.value.hostname)
  content         = "${cloudflare_dns_record.record[each.value.alias_of].name}.${data.cloudflare_zone.bowmans_site.name}"
  proxied         = each.value.cf_proxy_enabled
  ttl             = 1
}